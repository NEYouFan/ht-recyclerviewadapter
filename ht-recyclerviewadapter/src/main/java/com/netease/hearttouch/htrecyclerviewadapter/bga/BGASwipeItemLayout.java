...
/**
 * Created by zyl06 on 10/15/15.   Copy from :
 * <p>
...
public class BGASwipeItemLayout extends FrameLayout {
    ...
    private OnDragRadioChangedListener mOnDragRadioChangedListener = null;
    // 顶部视图
    ...
    // 移动过程中，底部视图的移动方式（拉出，被顶部视图遮住），默认是拉出
    ...
    private boolean fadeInOrOut = true;
    ...
    void setBottomModel(BottomModel bottomModel) {
        mBottomModel = bottomModel;
    }

    void setSpringDistance(int springDistance) {
        mSpringDistance = springDistance;
    }

    public void setOnDragRadiaoChangedListener(OnDragRadioChangedListener listener) {
        mOnDragRadioChangedListener = listener;
    }

    boolean isFadeInOrOut() {
        return fadeInOrOut;
    }

    void setFadeInOrOut(boolean fadeInOrOut) {
...
//        if (event.getPointerCount() == 1) {
        int activePointId = mDragHelper.getActivePointerId();
        if (mSwipeable && activePointId == ViewDragHelper.INVALID_POINTER) {
            mDragHelper.captureChildView(mTopView, 0);
        }
        try {
            mDragHelper.processTouchEvent(event);//多指滑动会崩溃
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        // 保证bottom容器和top容器在height方向都是撑满父view的
        topBottom = bottomBottom = Math.max(topBottom, bottomBottom);
    ...
    private void slideTo(int isOpen) {
        if (isOpen == 1) {
            ...
            if (mDelegate != null) {
                mDelegate.onBGASwipeItemLayoutOpened(this, 0);
            }
        } else {
            mBottomView.setVisibility(INVISIBLE);
            mCurrentStatus = Status.Closed;
            if (mDelegate != null) {
                mDelegate.onBGASwipeItemLayoutClosed(this, 0);
            }
        }
        ...
        if (mOnDragRadioChangedListener != null) {
            mOnDragRadioChangedListener.onDragRadioChanged(isOpen);
        }
    }
    ...
    private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            if (mOnDragRadioChangedListener != null) {
                mOnDragRadioChangedListener.onDragRadioChanged(mDragRatio);
            }
    ...
    private void dispatchSwipeEvent() {
        ...
        if (mCurrentStatus != preStatus) {
            ...
            } else if (mCurrentStatus == Status.Moving) {
                mBottomView.setVisibility(VISIBLE);
                if (mDelegate != null) {
                    mDelegate.onBGASwipeItemLayoutStartOpen(this, 0);
                }
            }
        }
    }
    ...
    public interface BGASwipeItemLayoutDelegate {
         ...
         */
        void onBGASwipeItemLayoutOpened(BGASwipeItemLayout swipeItemLayout, int position);
         ...
         * @param position default 0
         */
        void onBGASwipeItemLayoutClosed(BGASwipeItemLayout swipeItemLayout, int position);
         ...
         * @param position default 0
         */
        void onBGASwipeItemLayoutStartOpen(BGASwipeItemLayout swipeItemLayout, int position);
    }

    public class SimpleBGASwipeItemLayoutDelegate implements BGASwipeItemLayoutDelegate {
        @Override
